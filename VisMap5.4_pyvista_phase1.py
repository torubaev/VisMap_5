...
def VisualizeData(CENTERS, CUBdat, CUBdatESP, xx, yy, zz):
    ...
    plotter = pv.Plotter(window_size=(1500, 950))
    try:
...
def launch_gui():
    ...
    tk.Label(action_box, textvariable=APP_STATE["suggested_range_var"]).grid(row=1, column=0, columnspan=3, sticky="w", pady=(0, 8))

    tk.Label(action_box, text="Opacity").grid(row=2, column=0, sticky="w")
    ...
    tk.Label(action_box, text="Colormap").grid(row=3, column=0, sticky="w")
    ...
    APP_STATE["viewer_controls"] = [
        isovalue_entry, apply_btn, opacity_scale, cmap_menu, atoms_cb, bonds_cb, offset_cb, bg_btn, label_btn,
        save_btn, reset_btn, close_btn, gen_btn, scan_btn, clear_btn, kill_value_entry, kill_pm_entry, kill_btn,
        del_btn, sync_btn
    ]
    set_viewer_controls_state(False)
