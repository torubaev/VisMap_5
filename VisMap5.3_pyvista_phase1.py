...
def sync_main_controls_from_viewer():
    ...
    APP_STATE["suggested_range_var"].set(
        f"Suggested range: {VIEWER_STATE['dens_min']:.6g} to {VIEWER_STATE['dens_max']:.6g}"
    )
...
def launch_gui():
    ...
    entry_file = tk.Entry(frm, width=90)
    ...
    entry_mwfn = tk.Entry(frm, width=90)
    ...
    options = tk.LabelFrame(frm, text="Calculation options", padx=8, pady=8)
    options.grid(row=4, column=0, columnspan=2, sticky="we", pady=(4, 8))
    ...
    viewer_box = tk.LabelFrame(frm, text="Viewer controls (external PyVista window)", padx=8, pady=8)
    viewer_box.grid(row=5, column=0, columnspan=2, sticky="nsew", pady=(8, 8))
    viewer_box.grid_columnconfigure(1, weight=1)
    frm.grid_rowconfigure(5, weight=1)

    APP_STATE["suggested_range_var"] = tk.StringVar(value="Suggested range: n/a")
    ...
    tk.Label(viewer_box, text="Density isovalue").grid(row=0, column=0, sticky="w", pady=(0, 8))
    isovalue_entry = tk.Entry(viewer_box, textvariable=APP_STATE["isovalue_var"], width=16)
    isovalue_entry.grid(row=0, column=1, sticky="w", pady=(0, 8))
    isovalue_entry.bind("<Return>", viewer_apply_isovalue)
    apply_btn = tk.Button(viewer_box, text="Apply", command=viewer_apply_isovalue, width=10)
    apply_btn.grid(row=0, column=2, sticky="w", padx=(8, 0), pady=(0, 8))

    tk.Label(viewer_box, textvariable=APP_STATE["suggested_range_var"]).grid(row=1, column=0, columnspan=4, sticky="w", pady=(0, 12))

    tk.Label(viewer_box, text="Opacity").grid(row=2, column=0, sticky="w", pady=(0, 8))
    opacity_scale = tk.Scale(viewer_box, from_=0, to=100, orient="horizontal", resolution=1, command=viewer_update_opacity, showvalue=True, length=260)
    opacity_scale.set(50)
    opacity_scale.grid(row=2, column=1, columnspan=3, sticky="we", pady=(0, 8))
    APP_STATE["opacity_scale"] = opacity_scale

    tk.Label(viewer_box, text="Colormap").grid(row=3, column=0, sticky="w", pady=(0, 8))
    cmap_menu = tk.OptionMenu(viewer_box, APP_STATE["cmap_var"], "gist_rainbow", "turbo", "coolwarm", "RdBu_r", "plasma", "viridis", command=viewer_update_cmap)
    cmap_menu.grid(row=3, column=1, sticky="w", pady=(0, 8))

    atoms_cb = tk.Checkbutton(viewer_box, text="Show atoms", variable=APP_STATE["show_atoms_var"], command=viewer_toggle_atoms)
    atoms_cb.grid(row=4, column=0, sticky="w", pady=(0, 6))
    bonds_cb = tk.Checkbutton(viewer_box, text="Show bonds", variable=APP_STATE["show_bonds_var"], command=viewer_toggle_bonds)
    bonds_cb.grid(row=4, column=1, sticky="w", pady=(0, 6))

    save_btn = tk.Button(viewer_box, text="Save as...", command=viewer_save_as, width=12)
    save_btn.grid(row=5, column=0, sticky="w", pady=(4, 8))
    reset_btn = tk.Button(viewer_box, text="Reset view", command=viewer_reset, width=12)
    reset_btn.grid(row=5, column=1, sticky="w", pady=(4, 8))
    close_btn = tk.Button(viewer_box, text="Close viewer", command=close_viewer, width=12)
    close_btn.grid(row=5, column=2, sticky="w", pady=(4, 8))

    extrema_box = tk.LabelFrame(viewer_box, text="Extrema tools", padx=8, pady=8)
    extrema_box.grid(row=6, column=0, columnspan=4, sticky="we", pady=(8, 0))
    extrema_box.grid_columnconfigure(4, weight=1)

    gen_btn = tk.Button(extrema_box, text="Generate extrema", command=lambda: viewer_generate_extrema(False), width=16)
    gen_btn.grid(row=0, column=0, padx=(0, 6), pady=(0, 6), sticky="w")
    scan_btn = tk.Button(extrema_box, text="Scan near UA", command=lambda: viewer_generate_extrema(True), width=14)
    scan_btn.grid(row=0, column=1, padx=(0, 6), pady=(0, 6), sticky="w")
    clear_btn = tk.Button(extrema_box, text="Clear extrema", command=viewer_clear_extrema, width=14)
    clear_btn.grid(row=0, column=2, padx=(0, 6), pady=(0, 6), sticky="w")

    tk.Label(extrema_box, text="Kill value").grid(row=1, column=0, sticky="w")
    kill_value_entry = tk.Entry(extrema_box, textvariable=APP_STATE["kill_value_var"], width=10)
    kill_value_entry.grid(row=1, column=1, sticky="w")
    tk.Label(extrema_box, text="± range").grid(row=1, column=2, sticky="w")
    kill_pm_entry = tk.Entry(extrema_box, textvariable=APP_STATE["kill_pm_var"], width=10)
    kill_pm_entry.grid(row=1, column=3, sticky="w")
    kill_btn = tk.Button(extrema_box, text="Kill range", command=viewer_kill_range, width=12)
    kill_btn.grid(row=1, column=4, sticky="w")

    tk.Label(extrema_box, text="Extrema values").grid(row=2, column=0, columnspan=5, sticky="w", pady=(8, 4))
    extrema_text = tk.Text(extrema_box, width=90, height=12)
    extrema_text.grid(row=3, column=0, columnspan=5, sticky="we")
    extrema_text.config(state="disabled")
    APP_STATE["extrema_text"] = extrema_text

    APP_STATE["viewer_controls"] = [
        isovalue_entry, apply_btn, opacity_scale, cmap_menu, atoms_cb, bonds_cb, save_btn, reset_btn, close_btn,
        gen_btn, scan_btn, clear_btn, kill_value_entry, kill_pm_entry, kill_btn
    ]
    set_viewer_controls_state(False)

    status = tk.Label(frm, text="", anchor="w", justify="left", fg="blue")
    status.grid(row=6, column=0, columnspan=2, sticky="we", pady=(8, 0))
    ...
    tk.Button(frm, text="Run VisMap", command=run_clicked, width=18, height=2).grid(row=7, column=0, sticky="w", pady=(14, 0))
    tk.Button(frm, text="Quit", command=root.destroy, width=12).grid(row=7, column=1, sticky="e", pady=(14, 0))
