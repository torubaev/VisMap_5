...
def VisualizeData(CENTERS, CUBdat, CUBdatESP, xx, yy, zz):
    ...
    def rebuild_surface():
        ...
        state["surface_actor"] = plotter.add_mesh(
            ...
            smooth_shading=True,
            scalar_bar_args={"title": "ESP, kcal/mol"},
            name="esp_surface",
            render=False,
        )
        plotter.render()
    ...
    def update_isovalue(value):
        state["isovalue"] = float(value)
        rebuild_surface()
    ...
    def update_cmap(value):
        ...
        idx = max(0, min(idx, len(cmap_list) - 1))
        state["cmap_index"] = idx
        plotter.add_text(f"Colormap: {cmap_list[idx]}", position="upper_right", font_size=10, name="cmap_label")
        rebuild_surface()

    def toggle_atoms(flag):
        state["show_atoms"] = bool(flag)
        build_atoms()

    def toggle_bonds(flag):
        state["show_bonds"] = bool(flag)
        build_bonds()

    rebuild_surface()
    build_atoms()
    build_bonds()
    plotter.add_text(f"Colormap: {cmap_list[state['cmap_index']]}", position="upper_right", font_size=10, name="cmap_label")

    plotter.add_slider_widget(
        update_isovalue,
        rng=[float(np.min(CUBdat)), float(np.max(CUBdat))],
        value=float(state["isovalue"]),
        title="Density isovalue",
        pointa=(0.03, 0.10),
        pointb=(0.33, 0.10),
        style="modern",
    )
    ...
    plotter.add_slider_widget(
        update_cmap,
        rng=[0, len(cmap_list) - 1],
        value=state["cmap_index"],
        title="Colormap",
        pointa=(0.03, 0.22),
        pointb=(0.33, 0.22),
        style="modern",
    ...
    plotter.add_checkbox_button_widget(
        toggle_atoms,
        value=state["show_atoms"],
        position=(20, 190),
        size=24,
        border_size=1,
    )
    plotter.add_text("Atoms", position=(55, 193), font_size=10, viewport=True, name="atoms_label")

    plotter.add_checkbox_button_widget(
        toggle_bonds,
        value=state["show_bonds"],
        position=(20, 150),
        size=24,
        border_size=1,
    )
    plotter.add_text("Bonds", position=(55, 153), font_size=10, viewport=True, name="bonds_label")
